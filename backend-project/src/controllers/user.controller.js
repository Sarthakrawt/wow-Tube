...
// mean you can only modify the cookies in server side  that's it not from user side or fornt end
...
.json(
   ...
   const deleteCurrentUser = asyncHandler(async(req,res)=>{
      ...
      if(!(username === user.username && isPasswordCorrect)){
       throw new ApiError(400, "username and password is incorrect")
      }
    ...
    await User.deleteOne({username});
     ...
     .status(200)
     .json(new ApiResponse(200, req.user ,"user deleted succesfully"))
  ...
  console.log(coverImageLocalPath);
     if(!coverImageLocalPath){
      throw new ApiError(400, "coverImage  is not there ")
     }
...
// console.log(channel);
    ...
    const addWatchHistory = asyncHandler(async (req, res)=>{
      ...
      const userId = req.user._id;
       ...
       if(!video){
         throw ApiError(404, "video not founded")
       }
       const updatedUser = await User.findByIdAndUpdate(userId, 
         {$addToSet: {watchHistory: videoId}},{new : true}
       )
       return res.status(200)
       .json(new ApiResponse(200, updatedUser, "video is added to watch history"))
    })
    const deleteVideoFromWatchHistory = asyncHandler(async(req, res)=>{
      ...
      const user = await User.findByIdAndUpdate(userId, 
         {$pull: {watchHistory: videoId}},{new: true}
      )
      return res.status(200)
    .json(new ApiResponse(200,user, "video is removed from user"))
    })
